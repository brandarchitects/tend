...
const MODES: { value: ChatMode; label: string; desc: string }[] = [
  ...
  { value: "bestpractice", label: "Best Practice", desc: "Wie Top-Networker es machen — faktisch, praxisnah." },
  { value: "career", label: "Karriere", desc: "Positionierung, Standing, Jobwechsel — strategisch angehen." },
  { value: "message", label: "Nachricht", desc: "Kontakt + Anlass angeben — AI formuliert einen Entwurf." },
]
...
export function TendAIPanel() {
  ...
  async function sendToAI(content: string, isInit?: boolean) {
    // Read mode at call time to ensure we get the latest
    const currentMode = useChatStore.getState().activeMode
  ...
  function handleModeSwitch(mode: ChatMode) {
    ...
    setMode(mode)
    // Add a system-like message to indicate mode switch
    addMessage({
      role: "assistant",
      content: `*Modus gewechselt zu **${MODES.find(m => m.value === mode)?.label}**.*`,
      mode,
      timestamp: new Date().toISOString(),
    })
  }
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-bg-surface lg:hidden">
        <ChatContent
      ...
      {/* Desktop: side panel */}
      <div className="hidden lg:flex lg:w-[400px] lg:shrink-0 lg:flex-col lg:border-l lg:border-bg-subtle lg:bg-bg-surface">
        <ChatContent
          visibleMessages={visibleMessages}
          isThinking={isThinking}
          activeMode={activeMode}
...
interface ChatContentProps {
  visibleMessages: ReturnType<typeof useChatStore.getState>["messages"]
  isThinking: boolean
  activeMode: ChatMode
  input: string
...
function ChatContent({
  visibleMessages, isThinking, activeMode, input, setInput,
  handleSubmit, handleModeSwitch, setOpen, messagesEndRef, inputRef,
}: ChatContentProps) {
  return (
    <>
      {/* Header */}
      ...
      {/* Mode Ribbon */}
      <div className="flex gap-1.5 overflow-x-auto border-b border-bg-subtle px-3 py-2">
        {MODES.map((mode) => (
          <button
            key={mode.value}
            onClick={() => handleModeSwitch(mode.value)}
            className={cn(
              "shrink-0 rounded-pill px-3 py-1.5 text-[11px] font-medium transition-colors",
      ...
      {/* Mode description */}
      <div className="border-b border-bg-subtle px-4 py-1.5">
